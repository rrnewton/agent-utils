...
def compare_jobs_env_width(py: list[str], rs: list[str], rep: Report) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dagrun-cross-jobs-env-") as td:
        ...
        def guarded_channel_leg(label: str, channel: str, *, boxed: bool) -> None:
            outcomes: dict[str, Outcome] = {}
            ...
            for name, command in (("py", py), ("rs", rs)):
                ...
                if not boxed:
                    args.append("--unsafe-no-cgroups")
                env = {
                    "SAFE_CI_DAG_RUNNER_JOBS_ENV": channel,
                    "OBSERVED_PATH": str(output),
                    "DAGRUN_NO_STEP_LOGS": "1",
                }
                if boxed:
            ...
            if boxed and all(_boxing_capability_unavailable(out) for out in outcomes.values()):
                print(
                    "cross[dagrun]: SKIP boxed guarded jobs-env differential: "
                    "cgroup-v2 + a working systemd --user scope are unavailable"
                )
                rep.ok(f"{check}:capability-unavailable")
                return
            ...
            if (
                ...
                and all(
                    "cannot safely retain assigned width 1" in text
        ...
        guarded_channel_leg("unboxed-readonly", "BASHOPTS", boxed=False)
        guarded_channel_leg("unboxed-dynamic", "LINENO", boxed=False)
        guarded_channel_leg("boxed-readonly", "BASHOPTS", boxed=True)
        ...
        for leg, extra, phrase in (
            ...
            *(
                (
                    f"startup-control-{name.lower()}",
                    {"SAFE_CI_DAG_RUNNER_JOBS_ENV": name},
