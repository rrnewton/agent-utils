...
| `pr-landing-planner` | A conflict-graph + CI-aware, **advisory** pull-request landing planner. Over the open PRs targeting a base branch it builds the real merge-conflict graph (`git merge-tree`, with a file-overlap fallback) + ordering edges (base-stacking / ancestry, transitively reduced), **classifies each red CI** into `real` / `flaky` / `stale-required-check` / `evaluate-once-race` / `runner-outage` (the headline value), computes freshness (commits-behind) and hold reasons, partitions into **parallel-safe groups**, and recommends a per-PR action (`land-now` / `rebase-then-land` / `refire-stale-gate` / `escalate-runner-outage` / `refire-ci` / `hold-fix` / `wait`), ordered priority → size → age. It NEVER arms, refires, or merges anything. Subcommands `plan` (default) / `graph` / `status` / `quickstart`; `--format {human,json,actions}`. The `actions` format is **tick-hub-integrable** (Option B, zero tick-hub change): a tick reminder runs `pr-landing-planner plan --format actions` as a capturing gate and dispatches a landing skill. | 🐍 **Python-first** (mypy `--strict`, zero explicit `Any`). Pure core (graph / classify / plan / emit) + a pluggable `VcsHost` protocol; ships `GitHubHost` (`gh` + `git`, honoring `--net-wrapper`/`--gh-cmd`) and a `FakeHost` for deterministic tests + the `--fixture` demo. Nothing project-specific baked in — gate-check name, flaky signatures, priority source are all config. Content-identity guard aborts if a PR moves mid-collection. Generalizes DeepScry's `pr_interference.py` + hermit's `pr_conflict_graph.py`/`pr_status.py` (predecessors NOT yet retired). **Follow-ups:** a Rust port + `cross/` differential; retiring the predecessors once a caller adopts this. |
