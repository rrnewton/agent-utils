...
| `parallel-experiment-runner` | Run **N concurrent seed-sweep workers under RESOURCE CONTAINMENT** via `safe-ci-dag-runner`'s two-level cgroup-v2 scope — a chaos search / fuzz sweep / flaky-repro hunt / parameter scan expressed as one command template with a `{seed}` placeholder over a seed range. Fixes the failure where unbounded parallel experiments saturate the host (~470 concurrent processes) and starve their own measurements. A **resource box** against a BUG in our own code (leak / run-forever / fork-bomb), **not a security sandbox** — no seccomp, no user-namespace isolation. **Four-axis containment**, each mapped to a named failure: **cpu** ('run forever') a load-immune **CPU-second** budget from cgroup `cpu.stat` (`--cpu-timeout`, omit = honest **UNSET**, never wall-derived); **memory** ('leak') `memory.max`; **pids** ('fork bomb') `pids.max` — the only axis that stops PID exhaustion (`--pids`); **wall** a defence-in-depth backstop **derived at ~3× the CPU budget** when unset. Plus: **declared + enforced** concurrency resolved from lane ∩ live-host-capacity ∩ measured per-worker footprint via a `1→2→4` calibration ramp; a **derived** per-worker cost estimate up front (or explicit UNSET) plus measured wall/CPU actuals after; a **clean kill naming the breach and by how much** (`CPU-TIMEOUT` / `MEMORY-CAP` / `PIDS-CAP` / `TIMEOUT`) via setsid-proof `cgroup.kill` — a `pids.max` breach *contains* the fork (`EAGAIN`) rather than killing, so the denied-fork count is carried in-memory and the classifier names the fork-bomb even though the cpu/wall guard did the reaping. Each seed becomes one contained `Step`; **there is no second runner** — containment, teardown, and per-step measurement are all reused. Subcommands `run` / `plan-round` (dry width+DAG) / `quickstart`; `--format {human,json}`; drive by inline flags or `--spec sweep.json`. Containment is on by default (refuses with exit 3 rather than run uncontained; `--allow-cgroup-failure` opts out). | 🐍 **Python-first** (mypy `--strict`, zero explicit `Any`). Additive generalization of `safe-ci-dag-runner`; the pids axis rides an in-memory `StepOutcome` field, **not** a new CSV column, so the `cross/` profile-store schema stays byte-identical. Stable profile keys fold in mandatory identity dims (`backend`/`kernel_id`/`image_id`/`vcpu`/`guest_memory`/`accelerator`) so estimates stay apples-to-apples, and an unattributed run is *ephemeral* (calibrates but is not persisted). **Follow-ups:** a Rust port + `cross/` differential once a caller adopts it. |
