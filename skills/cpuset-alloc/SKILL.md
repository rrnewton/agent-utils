...
`$XDG_RUNTIME_DIR/safe-ci-dag-runner/core-reservations.json`, falling back under
`${TMPDIR:-/tmp}/safe-ci-dag-runner-$UID/`; `SAFE_CI_CORE_LEDGER` overrides it. Because `status`
...
The allocator is also integrated into `safe-ci-dag-runner` for DAG steps.
