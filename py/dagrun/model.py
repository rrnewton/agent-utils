...
# These names remain assignable in bash, so an assignment/readback probe alone cannot reject them,
# but assigning a decimal worker count changes shell startup or command-resolution semantics. The
# dynamic/readonly namespace (LINENO, RANDOM, BASHOPTS, SHELLOPTS, IFS, and peers) is deliberately
# NOT duplicated here: the final-boundary non-numeric capability probe owns that open-ended class.
