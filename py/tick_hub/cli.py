...
def _load_userguide() -> str:
    ...
    The guide is a real package resource (``tick_hub/USER_GUIDE.md``, declared as ``package-data``),
    generated by ``scripts/embed_userguides.py`` from the single source
    ``common/docs/tick-hub/USER_GUIDE.md``. Reading it via ``importlib.resources`` (NOT a path
    outside the package) is what makes ``--userguide`` work after ``pip install`` / from a wheel,
    where ``common/docs/`` is not shipped."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tick_p = sub.add_parser("tick", help="run one tick (emit HEALTH/ACTION/NOTE/ERROR lines)")
    tick_p.add_argument(
        "--config",
        required=True,
        metavar="FILE",
    ...
    state_p = sub.add_parser("state", help="validate + show the ops-state's own lines")
