...
running, start with the README's 60-second quickstart or `tick-hub quickstart`. This exact guide is
embedded in the package — `tick-hub --userguide` prints it (so it is available even from a
`pip install` where the repo docs are not shipped).
