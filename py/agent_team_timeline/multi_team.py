    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="agent-team-timeline-combine-") as raw_tmp:
        ...
        readme = (
            ...
            "Use `make open` to ask Python to open the browser. Use `make run-stats` "
            "to inspect recorded pipeline runs. Do not open `index.html` directly: browsers "
            "block the JSON fetch from `file://`.\n"
        )
        changed += int(
