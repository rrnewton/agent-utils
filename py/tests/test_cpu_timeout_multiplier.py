...
def breach(**kwargs) -> str:
    ...
    base.update(kwargs)
    return step_failure_reason(**base)
...
class TestUnityIsAStrictNoOp:
    ...
    def test_effective_equals_canonical_without_a_multiplier(self):
...
class TestScaling:
    ...
    def test_the_small_default_scales_too(self):
    ...
    def test_scaling_rounds_to_whole_seconds(self):
...
class TestScalingCannotBecomeAnOptOut:
    ...
    def test_a_live_budget_never_rounds_down_to_zero(self):
    ...
    def test_a_negative_canonical_is_treated_as_disabled_not_negative(self):
...
class TestBreachAttribution:
    ...
    def test_scaled_breach_names_canonical_multiplier_and_platform(self):
        message = breach(
            cpu_timeout=60,
    ...
    def test_the_enforced_number_still_leads(self):
...
class TestResolution:
    ...
    def test_a_nonpositive_environment_value_is_refused(self, raw):
        with pytest.raises(ValueError):
            resolve_cpu_timeout_multiplier(None, {CPU_TIMEOUT_MULTIPLIER_ENV: raw})

    def test_a_nonpositive_explicit_value_is_refused(self):
        with pytest.raises(ValueError):
...
class TestTheGraphStaysCanonical:
    ...
    def test_the_step_field_is_never_rewritten(self):
        s = step(cpu_timeout=30)
        assert effective_cpu_timeout(s, DEFAULT_SMALL_CPU_TIMEOUT, 2.0) == 60
