...
@pytest.mark.parametrize("name", _SHELL_STARTUP_CONTROL_NAMES)
def test_shell_startup_control_name_is_refused(name: str) -> None:
    with pytest.raises(ValueError, match="shell startup/control variable"):
        resolve_jobs_env(env={JOBS_ENV_ENV: name})


def test_dynamic_shell_parameters_are_deferred_to_the_runtime_probe() -> None:
    """The open-ended shell namespace is tested by behavior, not a brittle denylist."""
    assert resolve_jobs_env("LINENO", env={}) == "LINENO"
    assert resolve_jobs_env("BASHOPTS", env={}) == "BASHOPTS"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"default_jobs_env": "BASH_ENV", "steps": []}, "default_jobs_env"),
        (
            {
                "steps": [
        ...
        (
            {"steps": [{"group": "g", "job": "j", "cmd": "true", "jobs_env": "ENV"}]},
            "steps[0].jobs_env",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    ids=("unboxed-bash-readonly", "unboxed-bash-dynamic", "boxed-wrapper-readonly"),
)
def test_unassignable_jobs_env_refuses_before_guest_command(
    ...
    assert "cannot safely retain assigned width 1" in result.outcomes[0].summary
...
@pytest.mark.parametrize("jobs_env", ("BASH_ENV", "ENV"))
def test_programmatic_startup_control_channel_refuses_before_spawn(
    tmp_path: Path, jobs_env: str
) -> None:
    marker = tmp_path / f"spawned-{jobs_env}"
    cfg = DagConfig(
        steps=(Step("g", "j", "", f"touch {shlex.quote(str(marker))}"),),
        default_jobs_env=jobs_env,
    )
