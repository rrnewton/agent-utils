...
- The zombies were real and genuinely held **PID slots** at **zero CPU and zero memory** — a box
  ...
  scenarios stranded **zero** workers). The cause was a **live, hung `hermit run --strict --verify`**
...
What actually held the zombies open was a **live, hung** `hermit run --strict --verify` — its main
...
```json
{
  ...
  "command": ["hermit", "run", "--chaos", "--seed", "{seed}", "./demo"],
