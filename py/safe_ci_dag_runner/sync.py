...
from safe_ci_dag_runner.summary import (
    DEFAULT_MAX_BUCKETS,
    DEFAULT_RESERVOIR_K,
    Summary,
    SummaryError,
...
class GitBranchBackend:
    ...
    def _fresh_checkout(self) -> str:
        ...
        work = tempfile.mkdtemp(prefix="scdr-sync-git-")
        ...
        _run_git(
            ["config", "user.name", "safe-ci-dag-runner"], work, timeout=self.timeout, check=True
        )
        return work
...
class GitHubArtifactsBackend:
    ...
    def download(self, machine_id: str, container_class: str) -> Summary:
        ...
        if not isinstance(artifact_id, int):
            return summarylib.empty(machine_id, container_class)
        with tempfile.TemporaryDirectory(prefix="scdr-sync-gha-") as tmp:
