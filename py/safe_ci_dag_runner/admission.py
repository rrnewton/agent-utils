...
:mod:`safe_ci_dag_runner.reservation`: a durable, ``flock``-serialized file that every runner on
...
def _env_bytes(name: str) -> int | None:
    ...
    if value < 0:
        print(
            f"[safe-ci-dag-runner] WARNING: {name}='{raw}' is not a non-negative integer number "
