...
def _run(cfg: DagConfig, ns: argparse.Namespace, c: Palette) -> int:
    ...
    # --stress implies --keep-going: a failing copy must not cancel or prevent later independent
    # copies from running, or the per-copy ratio (the whole point) becomes a partial measurement.
