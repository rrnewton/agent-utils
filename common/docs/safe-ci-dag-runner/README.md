...
- **cgroup boxing is ON by default (both builds).** `run` boxes each step: it re-execs the run inside
  ...
  old opt-in `--cgroups` flag has been **removed** (it enforced nothing once boxing became the
  default); passing it is now a hard error naming the replacement. (The Python *library* entry point
...
- **The Rust crate is at full parity.** `run`, `list`, `ascii`, `dot`, `json`, and `quickstart` all
  ...
  **also writes per-step + whole-run perf CSVs via `--perf-dir`**; the earlier Python-only gap for
  Linux cgroup boxing and perf logging has been closed. The removed `--cgroups` flag hard-errors in both.
