...
default when the document omits it. Shell startup/control names are refused,
and every other channel must preserve a nonnumeric probe across the wrapper
shell and child boundaries before the numeric width is delivered. Empty
effective channels prevent rewriting.
