...
`--rescued-current-tip NAME=refs/rescue/EXACT-REF --landed-commit NAME=40_HEX_SHA` mode
verifies fresh remote readback, fresh agreement between the local landed tracking ref and its
remote branch, recorded-head ancestry, whole-tree equality between the rescued tip and supplied
landed commit, and ancestry from that exact commit to verified landed main. It requires both the
checkout path and stale Git registration to be absent and performs no Git or path mutation. The
read-only plan prints every bound checkout, tip, rescue ref, and landed commit. Without that
complete explicit proof, the original exact-recorded-HEAD behavior remains unchanged.
