...
use safe_ci_dag_runner::model::{
    canonical_cpu_timeout, resolved_wall_timeout, DEFAULT_SMALL_CPU_TIMEOUT, DEFAULT_STEP_TIMEOUT,
    WALL_CPU_BACKSTOP_FACTOR,
};
use safe_ci_dag_runner::{
    dag_from_json, dag_to_json, steps_violating_run_timeout, DagConfig, Step,
};
...
fn a_declared_cpu_budget_derives_the_backstop() {
    assert_eq!(resolved_wall_timeout(&step(0, 7), 0, 1.0), 21);
}
...
fn the_derivation_tracks_the_platform_scaled_budget() {
    ...
    assert_eq!(resolved_wall_timeout(&step(0, 4), 0, 2.5), 30);
}
...
fn the_run_budget_ordering_is_checked_on_the_resolved_value() {
    ...
    let cfg = DagConfig {
        steps: vec![step(0, 100)], // derives a 300-second wall backstop
        ..Default::default()
    };
    assert_eq!(
        steps_violating_run_timeout(&cfg, 200),
        vec![("g.a".to_string(), 300)]
    );
    assert!(steps_violating_run_timeout(&cfg, 400).is_empty());
}
...
#[test]
fn a_step_the_derivation_brings_under_the_run_budget_is_not_refused() {
    ...
    assert!(steps_violating_run_timeout(&cfg, 60).is_empty());
}
