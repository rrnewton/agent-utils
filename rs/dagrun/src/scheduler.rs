...
fn checked_jobs_env_export(name: &str, value: &str) -> String {
    ...
    let failure = format!(
        "dagrun: ERROR: jobs environment variable {name} cannot safely retain assigned width {value}; \
...
fn run_step(ctx: StepCtx) {
    ...
    if let Some((name, _value)) = &jobs_env {
        // Do not expose a caller-selected key to bash startup. Startup-sensitive names can act
        // before the command string reaches its first guard, while dynamic parameters can be
        // rewritten during startup. The checked final-boundary export is the sole authority.
        cmd.env_remove(name);
    }
...
mod tests {
    ...
    fn unboxed_bash_readonly_jobs_env_refuses_before_guest_command() {
        ...
        assert!(result.outcomes[0]
            .summary
            .contains("cannot safely retain assigned width 1"));
        assert!(!output.exists(), "the guest command must not run");
    }

    #[test]
    fn unboxed_bash_dynamic_jobs_env_refuses_before_guest_command() {
        let output = jobs_env_output("unboxed_dynamic");
