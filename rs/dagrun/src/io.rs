...
"#;
    ...
    fn malformed_jobs_env_fields_are_refused_by_location() {
        ...
        let top_control = dag_from_json(r#"{"default_jobs_env":"BASH_ENV","steps":[]}"#)
            .unwrap_err()
            .to_string();
        assert!(top_control.contains("<root>.default_jobs_env"), "{top_control}");
        assert!(
            top_control.contains("shell startup/control variable"),
            "{top_control}"
        );

        let step_control = dag_from_json(
            r#"{"steps":[{"group":"g","job":"j","cmd":"true","jobs_env":"ENV"}]}"#,
        )
        .unwrap_err()
        .to_string();
        assert!(step_control.contains("steps[0].jobs_env"), "{step_control}");
        assert!(
            step_control.contains("shell startup/control variable"),
            "{step_control}"
        );
    }
